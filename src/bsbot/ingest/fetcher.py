...
from bsbot.index.store import FetchRecord, Store
...
class Fetcher:
    ...
    async def fetch(self, url: str, *, moodle_timemodified: int | None = None) -> FetchResult:
        now = self._clock()
        record = self._store.fetch_record(url)
        ...
        if cached:
            ...
            if not moved and fresh:
                return FetchResult(url, FetchOutcome.CACHED, sha256=record.sha256)
    ...
    async def _download(
        ...
        *,
        ...
        try:
            async with (
                ...
                if response.status_code == 304:  # AC-6
                    self._store.touch_fetch(url, checked_at=now)
                    return FetchResult(
                        url,
        ...
        return FetchResult(url, outcome, sha256=digest, filename=filename)

    def _fail(self, url: str, error: str, now: int) -> FetchResult:
        self._store.record_fetch_failure(url, error=error, checked_at=now)
